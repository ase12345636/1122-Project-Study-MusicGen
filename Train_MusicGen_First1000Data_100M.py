...
from Config.Config_100M import device, d_model, nheads, nlayer, d_hid, dropout, num_epoch, batch_multiplier, lr, betas, eps, PATH, loss_function_max_length
...
# model.load_state_dict(torch.load(Path(PATH+mode+"Model_Best_300Data_10M.h5")))
# summary(model)
...
mem_train_folder_path = Path(
    "Dataset/PreproccessedData/train/MemData")
...
for epoch in range(num_epoch):
    ...
    for file_part in range(len(shuffle_data)):
        ...
        for batch in range(len(tgt_train_data)):
            ...
            with autocast():
                ...
                mem = mem_train_data[batch, :8, :]
                tgt = tgt_train_data[batch, :8, :, :-1]
                tgt_gt = tgt_train_data[batch, :8, :, 1:]
            ...
            if count == 0 or file_part+1 == len(mem_train_dirs):
                ...
                if min_loss == last_loss or iteration == 1:
                    torch.save(model.state_dict(), Path(
                        PATH+mode+"Model_Best_First300CleanData_100M.h5"))
                    torch.save(model_ema.model.state_dict(), Path(
                        PATH+mode+"Model_Ema_Best_First300CleanData_100M.h5"))
    ...
    #             mem = mem_validation_data[batch, :10, :]
    #             tgt = tgt_validation_data[batch, :10, :, :-1]
    #             tgt_gt = tgt_validation_data[batch, :10, :, 1:]
    ...
    #             if min_loss == last_loss or iteration == 1:
    #                 torch.save(model.state_dict(), Path(
    #                     PATH+mode+"Model_Best_1000Data_10M.h5"))
    #                 torch.save(model_ema.model.state_dict(), Path(
    #                     PATH+mode+"Model_Ema_Best_1000Data_10M.h5"))

    # Save last model
    torch.save(model.state_dict(), Path(
        PATH+mode+"Model_Last_First300CleanData_100M.h5"))
    torch.save(model_ema.model.state_dict(), Path(
        PATH+mode+"Model_Ema_Last_First300CleanData_100M.h5"))
...
plt.savefig(Path("loss_history_First300CleanData_100M.png"))
plt.show()

plt.plot(np.array(iteration_history),
         np.array(lr_history), label='training loss')
plt.savefig(Path("lr_history_First300CleanData_100M.png"))
plt.show()
