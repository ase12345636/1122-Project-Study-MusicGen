...
CFGConditions = tp.Union[ConditionTensors,
                         tp.Tuple[ConditionTensors, ConditionTensors]]
...
class LMModel(StreamingModule):
    ...
    def _init_weights(self, weight_init: tp.Optional[str], depthwise_init: tp.Optional[str], zero_bias_init: bool):
        ...
        for emb_layer in self.emb:
            init_layer(emb_layer, method=weight_init,
                       init_depth=None, zero_bias_init=zero_bias_init)
        ...
        for layer_idx, tr_layer in enumerate(self.transformer.layers):
            ...
            init_fn = partial(init_layer, method=weight_init,
                              init_depth=depth, zero_bias_init=zero_bias_init)
    ...
    def forward(self, sequence: torch.Tensor,
        ...
        logits = torch.stack([self.linears[k](out)
                             for k in range(K)], dim=1)  # [B, K, S, card]
    ...
    def compute_predictions(
        ...
        logits = model(sequence_codes, conditions,
                       condition_tensors, stage=stage)  # [B, K, S, card]
    ...
    def _sample_next_token(self,
        ...
        if two_step_cfg and cfg_conditions != {}:
            ...
            cond_logits = model(sequence, conditions=[],
                                condition_tensors=condition_tensors)
            state = self.get_streaming_state()
            self.set_streaming_state(unconditional_state)
            uncond_logits = model(sequence, conditions=[],
                                  condition_tensors=null_condition_tensors)
            unconditional_state.update(self.get_streaming_state())
            self.set_streaming_state(state)
            logits = uncond_logits + \
                (cond_logits - uncond_logits) * self.cfg_coef
        else:
            ...
            if condition_tensors:
                cond_logits, uncond_logits = all_logits.split(
                    B, dim=0)  # [B, K, T, card]
                logits = uncond_logits + \
                    (cond_logits - uncond_logits) * cfg_coef
            else:
                logits = all_logits
    ...
    def generate(self,
            ...
            for offset in range(start_offset_sequence, gen_sequence_len):
                ...
                if check:
                    ...
                    assert (curr_sequence == torch.where(
                        curr_mask, curr_sequence, self.special_token_id)).all()
        ...
        assert (
            gen_sequence == torch.where(mask[None, ...].expand(
                B, -1, -1), gen_sequence, self.special_token_id)
        ).all()
