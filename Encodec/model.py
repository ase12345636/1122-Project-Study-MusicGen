...
# ! EnCodec model 
# model.py : 
#    1. LMMmodel :用來估計每一個codebook entry 的機率
#    2. EncodecModel: encoder -> RVQ -> decoder
...
class EncodecModel(nn.Module):
    ...
    # 使用語言模型進一步壓縮，使用的是pretrain model
...
def test():
    ...
    for model_name, bw in product(models.keys(), bandwidths):
        ...
        wav ,sr= torchaudio.load(f"test_{audio_suffix}.wav",backend="soundfile")
