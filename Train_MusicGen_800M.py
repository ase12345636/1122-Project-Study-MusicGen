...
# model.load_state_dict(torch.load(Path(PATH+mode+"Model_Best_AllData_30M.h5")))
# summary(model)
...
for epoch in range(num_epoch):
            ...
            if count == 0 or file_part+1 == len(mem_train_dirs):
                ...
                if min_loss == last_loss or iteration == 1:
                    torch.save(model.state_dict(), Path(
                        PATH+mode+"Model_Best_First600CleanData_800M.h5"))
                    torch.save(model_ema.model.state_dict(), Path(
                        PATH+mode+"Model_Ema_Best_First600CleanData_800M.h5"))
    ...
    #             mem = mem_validation_data[batch, :6, :]
    #             tgt = tgt_validation_data[batch, :6, :, :-1]
    #             tgt_gt = tgt_validation_data[batch, :6, :, 1:]
    ...
    #             if min_loss == last_loss or iteration == 1:
    #                 torch.save(model.state_dict(), Path(
    #                     PATH+mode+"Model_Best_First1000Data_600M.h5"))
    #                 torch.save(model_ema.model.state_dict(), Path(
    #                     PATH+mode+"Model_Ema_Best_First1000Data_600M.h5"))

    # Save last model
    torch.save(model.state_dict(), Path(
        PATH+mode+"Model_Last_First600CleanData_800M.h5"))
    torch.save(model_ema.model.state_dict(), Path(
        PATH+mode+"Model_Ema_Last_First600CleanData_800M.h5"))
...
plt.savefig(Path("loss_history_First600CleanData_800M.png"))
plt.show()

plt.plot(np.array(iteration_history),
         np.array(lr_history), label='training loss')
plt.savefig(Path("lr_history_First600CleanData_800M.png"))
plt.show()
