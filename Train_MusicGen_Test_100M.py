...
from Config.Config_10M import mode_table
from Config.Config_10M import device, d_model, nheads, nlayer, d_hid, dropout, num_epoch, batch_multiplier, lr, betas, eps, PATH, loss_function_max_length
    ...
    for file_part in range(len(shuffle_data)):
        ...
        for batch in range(len(tgt_train_data)):
            ...
            with autocast():
                ...
                mem = mem_train_data[batch, :6, :]
                tgt = tgt_train_data[batch, :6, :, :-1]
                tgt_gt = tgt_train_data[batch, :6, :, 1:]
...
plt.savefig(Path("loss_history_First600Data_10M.png"))
plt.show()
